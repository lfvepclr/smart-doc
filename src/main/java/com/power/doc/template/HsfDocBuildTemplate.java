...
public class HsfDocBuildTemplate implements IDocBuildTemplate<RpcApiDoc> {
    ...
    public List<RpcApiDoc> getApiData(ProjectDocConfigBuilder projectBuilder) {
        ...
        JavaProjectBuilder javaProjectBuilder = projectBuilder.getJavaProjectBuilder();
        for (JavaClass cls : javaProjectBuilder.getClasses()) {
            ...
            Optional<JavaAnnotation> classAnnotationOptional = cls.getAnnotations().stream().filter(e -> HSF_PROVIDER.equals(e.getType().getCanonicalName())).findFirst();
            if (!classAnnotationOptional.isPresent()) {
                continue;
            }
            JavaAnnotation hsfProviderAnnotation = classAnnotationOptional.get();
            TypeRef serviceInterface = (TypeRef) hsfProviderAnnotation.getProperty("serviceInterface");
            JavaClass hsfProviderInterface = (JavaClass) serviceInterface.getType();
            String strOrder = JavaClassUtil.getClassTagsValue(hsfProviderInterface, DocTags.ORDER, Boolean.TRUE);
            order++;
            if (ValidateUtil.isNonnegativeInteger(strOrder)) {
                order = Integer.parseInt(strOrder);
                setCustomOrder = true;
            }
            List<JavaMethodDoc> apiMethodDocs = buildServiceMethod(hsfProviderInterface, apiConfig, projectBuilder);
            this.handleJavaApiDoc(hsfProviderInterface, apiDocList, apiMethodDocs, order, projectBuilder);
        }
        // sort
    ...
    public List<JavaMethodDoc> buildServiceMethod(final JavaClass cls, ApiConfig apiConfig, ProjectDocConfigBuilder projectBuilder) {
        ...
        for (JavaMethod method : methods) {
            ...
            apiMethodDoc.setDesc(DocUtil.getEscapeAndCleanComment(defaultIfNull(method.getComment(), method.getName())));
    ...
    private static final String HSF_PROVIDER = "com.alibaba.boot.hsf.annotation.HSFProvider";

    public boolean checkHsfInterface(JavaClass cls) {
        List<JavaAnnotation> classAnnotations = cls.getAnnotations();
        for (JavaAnnotation annotation : classAnnotations) {
            String name = annotation.getType().getCanonicalName();
            if (HSF_PROVIDER.equals(name)) {
                return true;
            }
        }
        return false;
    ...
    private String getComment(JavaClass cls) {
        String comment = cls.getComment();
        if (StringUtil.isEmpty(comment) && cls.getTags() != null) {
            Optional<String> description = cls.getTags().stream()
                    .filter(e -> "description:".equalsIgnoreCase(e.getName()))
                    .map(DocletTag::getValue).findFirst();
            if (description.isPresent()) {
                comment = description.get();
            }
        }
        if (StringUtil.isEmpty(comment)) {
            comment = cls.getSimpleName();
        }
        return comment;
    }
